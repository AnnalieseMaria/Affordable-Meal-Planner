...
function getDateStrings(startDate, timeframe) {
...
function getPackageTotalAmount(price) {
    return price.servingsPerContainer * price.servingSize;
}
function getBatchesNeeded(recipe, familySize) {
    return Math.ceil(familySize / recipe.recipeServings);
}
function usePantryIngredient(pantry, ingredient, priceLookup, batches) {
...
function marginalCost(recipe, pantry, priceLookup, batches) {
    let cost = 0;
...
function cheapestPossibleCost(pool, familySize, pantry, priceLookup) {
    if (pool.length === 0)
        return 0;
...
function cheapestPossibleCostExcluding(pool, excludeToday, familySize, pantry, priceLookup) {
...
function filterRecipes(recipes, cuisines, dietType, slot) {
...
function pickAffordableVariedMeal(pool, mealType, familySize, pantry, priceLookup, usedIds, leftovers, remainingBudget, reserveForRest, excludeToday, fairShareCeiling) {
    ...
    const leftoverIndex = leftovers.findIndex(l => l.mealType === mealType && l.remainingServings >= familySize && !excludeToday.has(l.recipe.recipeId));
    if (leftoverIndex !== -1) {
    ...
    if (poolExcludingToday.length === 0)
    ...
    const scoreRecipes = (recipes) => recipes.map(recipe => {
        const batches = getBatchesNeeded(recipe, familySize);
...
function generateMealPlan(allRecipes, cuisines, dietType, familySize, startDate, timeframe, priceLookup, weeklyBudget) {
    ...
    dates.forEach((date, index) => {
        ...
        const dailyMinCost = cheapestPossibleCost(breakfastPool, familySize, pantry, priceLookup) +
        ...
        const { recipe: breakfast, cost: bCost } = pickAffordableVariedMeal(breakfastPool, "Breakfast", familySize, pantry, priceLookup, usedIds.breakfast, leftovers, weeklyBudget - weekSpend, breakfastReserve, excludeToday, breakfastFairShare);
        weekSpend += bCost;
        totalSpend += bCost;
        if (breakfast)
            excludeToday.add(breakfast.recipeId);
        ...
        const { recipe: lunch, cost: lCost } = pickAffordableVariedMeal(lunchPool, "Lunch", familySize, pantry, priceLookup, usedIds.lunch, leftovers, weeklyBudget - weekSpend, lunchReserve, excludeToday, lunchFairShare);
        weekSpend += lCost;
        totalSpend += lCost;
        if (lunch)
            excludeToday.add(lunch.recipeId);
        // Dinner only needs to reserve future days
        const dinnerFairShare = (weeklyBudget - weekSpend) / (mealsRemainingInWeek - 2) * GENEROSITY;
        const { recipe: dinner, cost: dCost } = pickAffordableVariedMeal(dinnerPool, "Dinner", familySize, pantry, priceLookup, usedIds.dinner, leftovers, weeklyBudget - weekSpend, reserveForFutureDays, excludeToday, dinnerFairShare);
        weekSpend += dCost;
        totalSpend += dCost;
        if (dinner)
            excludeToday.add(dinner.recipeId);
        plan.push({ date, breakfast, lunch, dinner });
        dayInWeek++;
    });
    return { plan, pantry, totalSpend };
}
function generateShoppingList(pantry, priceLookup) {
    const list = [];
...
const { plan, pantry, totalSpend } = generateMealPlan(sampleRecipes, testCuisines, testDietType, testFamilySize, testStartDate, testTimeframe, samplePriceLookup, testWeeklyBudget);
console.log("=== MEAL PLAN ===");
plan.forEach((day, i) => {
    const weekLabel = `(week ${Math.floor(i / 7) + 1})`;
    console.log(`${day.date} ${weekLabel} | Breakfast: ${day.breakfast?.name ?? "—"} | Lunch: ${day.lunch?.name ?? "—"} | Dinner: ${day.dinner?.name ?? "—"}`);
});
